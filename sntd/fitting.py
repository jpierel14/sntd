...
def single_model_series_delays(curves,model,vparam_names,shared_parameters,referenceImage=None,fix_time=[],fix_magnification=[],
                                                ...
                                                nest_kwargs={},minsnr=0,no_bound_needed=[],**kwargs):
        if curves.series.table is None:
        ...
        if kwargs.get('ncpu',multiprocessing.cpu_count())>1:
                with dynesty.pool.Pool(kwargs.get('ncpu',multiprocessing.cpu_count()), loglike_color, prior_transform,
                        ...
                        sampler = dynesty.NestedSampler(pool.loglike, pool.prior_transform,
                                                                ndim, pool = pool,nlive=npoints,sample='rwalk')
...
def loglike_series(parameters,models,shared_indices,shared_vindices,unshared_indices,unshared_vindices,
        ...
        for i,mod in enumerate(mods):
                ...
                cov = image_data_dict[image_names[i]]['fluxerr']**2

                if modelcov:
                        temp = deepcopy(mod)
                        if 'x1' in temp.param_names:
                                temp.set(x1=0)
                        if 'c' in temp.param_names:
                                temp.set(c=0)

                        mfluxerr = np.diagonal(temp._bandflux_rcov(image_data_dict[image_names[i]]['band'],
                                image_data_dict[image_names[i]]['time']))*(mod_flux**2)

                        #print(np.median(mfluxerr),np.median(np.sqrt(cov)))
                        cov += mfluxerr**2
                ...
                #print(mod_flux)
                chisq += np.sum((image_data_dict[image_names[i]]['flux']-mod_flux)**2/cov)
                #print(chi)
        ...
        #sys.exit()
        #print(chisq)
        return -.5*chisq - np.sum(np.log(np.sqrt(cov))) - 0.5 * len(mod_flux) * np.log(2 * np.pi)
...
def series_nest(data,model,vparam_names,bounds,shared_parameters,unshared_parameters,use_MLE=False,band_systematics={},
        ...
        iparam_names = [key for key in vparam_names if key in ppfs or key in no_bound_needed]

        ppflist = [ppfs[key] for key in iparam_names if key not in no_bound_needed]
        npdim = len(iparam_names)  # length of u
        ndim = len(vparam_names)  # length of v
        tied = {}
        # Check that all param_names either have a direct prior or are tied.
        for name in vparam_names:
                if name in iparam_names or name in no_bound_needed or name in tied:
                        continue
        ...
        if kwargs.get('ncpu',multiprocessing.cpu_count())>1:
                if prior_transform_func is None:
                        prior_transform_func = prior_transform
                ...
                with dynesty.pool.Pool(kwargs.get('ncpu',multiprocessing.cpu_count()), loglike_series, prior_transform_func,
                        ...
                        sampler.run_nested(maxiter=maxiter,maxcall=maxcall,dlogz=.01)
        else:
                if prior_transform_func is None:
                        prior_transform_func = prior_transform_inner
                sampler = dynesty.NestedSampler(chisq_likelihood, prior_transform_func, ndim, nlive = npoints)
                sampler.run_nested(maxiter=maxiter,maxcall=maxcall)
        ...
        for i,mod in enumerate(models):
                ...
                if i==ref_im:
                        ...
                        mod.parameters[1] = best_params[t0_indices[ref_im]]-best_params[t0_indices[i]]
...
def fit_data(curves=None, snType='Ia', bands=None, models=None, params=None, bounds={}, ignore=None, constants={}, ignore_models=[],
        ...
        if not isinstance(args['curves'],list):
        ...
        if not isinstance(args['curves'],list):
                ...
                for i in range(len(args['curves'])):
                        args['curves'][i].micro_color_offset = micro_color_offset
                        args['curves'][i].upper_limit_dict = upper_limit_dict
                                ...
                                for pyfile in pyfiles:
                                        ...
                                        for i in range(len(method)):
                                                ...
                                                for par, val in locs.items():
                                                        ...
                                                        elif str(par)=='models' and val in __sntd_model_list__:
                                                                sntd_command += str(par) + '= sncosmo.Model(sntd.models.BAYESNSource(all_dat[i].bayesn_filters)),'
